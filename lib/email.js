 ...
 *
 * This used to be constructed inside sendEmail, so the template directory was
 * re-resolved and every template re-compiled for each individual message.
 */
let mailer = null;

/**
 * @returns {object} the shared email-templates instance
 */
function getMailer() {
  if (!mailer) {
    mailer = new Email({
...
/**
 * Reject anything that is not a plausible single address.
 *
 * Addresses are built by concatenating a username onto the configured domain.
 * In production that username comes from the directory, but the dev login
 * accepts any typed string, so a username containing a comma or a newline would
 * otherwise turn into a second recipient or a header injection attempt.
 *
 * @param {string} address
 * @returns {boolean}
 */
const isPlausibleAddress = (address) =>
...
const getAdminAddresses = () => {
  if (!emailConfig || !emailConfig.emailDomain) {
    // Guarded because the completion path already checked this and the admin
    // path did not: without a domain the addresses become bare usernames and
    // every notification throws.
    console.error("email.emailDomain is not configured; cannot notify admins.");
    return [];
  }
  const adminList = emailAdmins || admins || [];
  return adminList
    .map((username) => username + emailConfig.emailDomain)
    .filter((address) => {
      if (isPlausibleAddress(address)) return true;
      console.error(`Skipping implausible admin address: ${address}`);
      return false;
    });
};
...
emailService.newRequest = async (request) => {
  ...
  try {
    ...
    // Rethrown so the caller can tell the user the notification did not go
    // out. Swallowing it meant a request could save, return a clean success,
    // and nobody ever hear about it.
    throw err;
  }
};
...
/**
 * @param {object} request - The request object
 * @param {string} [reason] - What actually changed, in a sentence. The template
 *   used to assert a status change regardless of the trigger, so a note or an
 *   assignment told the submitter their status had changed when it had not.
 */
emailService.updatedRequest = async (request, reason) => {
  ...
  if (request.createdBy && emailConfig.emailDomain) {
    ...
    if (!isPlausibleAddress(creatorEmail)) {
      console.error(`Skipping implausible creator address: ${creatorEmail}`);
    } else if (!addresses.includes(creatorEmail)) {
