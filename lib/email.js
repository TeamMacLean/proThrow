...
Email.newRequest = (request) => {
  ...
  const newOrder = transporter.templateSender(
    new EmailTemplate(path.join(templatesDir, "new-request")),
    {
...
Email.updatedRequest = (request) => {
  ...
  const updatedOrder = transporter.templateSender(
    new EmailTemplate(path.join(templatesDir, "updated-request")),
    {
      from: config.email.from,
    }
  );
...
Email.requestComplete = (request) => {
  ...
  const requestComplete = transporter.templateSender(
    new EmailTemplate(path.join(templatesDir, "request-complete")),
    {
      from: config.email.from,
    }
  );
