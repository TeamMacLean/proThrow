...
/**
 * Copy a schema default before handing it to a document.
 *
 * Array and object defaults are declared once on the schema, so returning them
 * by reference let one document's mutation leak into every other document that
 * relied on the same default.
 *
 * @param {*} value
 * @returns {*}
 */
  ...
  string: () => ({
    ...
    validate: function (value) {
      if (value === undefined || value === null) {
        ...
        return cloneDefault(this._default);
      }
  ...
  const relationPromises = Object.entries(relations).map(
    async ([relationName, relationConfig]) => {
      ...
      try {
        if (relationType === "hasMany") {
          ...
          if (linkValue) {
            ...
            try {
              ...
              // Fallback to a full scan if the index is missing. Logged because
              // this silently turns an indexed lookup into an O(n) scan for
              // every joined document, which only shows up as the table grows.
              console.warn(
                `Index ${foreignKey} missing on ${relatedModel.tableName}; falling back to a table scan. Run "yarn check:indexes".`
              );
              relatedDocs = await r
                .table(relatedModel.tableName)
        ...
        // Rethrown rather than swallowed. Leaving the relation unset made a
        // transient failure indistinguishable from "this request has no
        // samples", and the edit path reads that as permission to insert every
        // submitted row afresh - duplicating the children while the originals
        // stay behind, permanently.
        throw new Error(
          `Could not load ${relationName} for ${Model.tableName}: ${err.message}`
        );
      }
    }
  );
...
function createModel(tableName, schema) {
  ...
  /**
   * The names of every relation declared on this model.
   *
   * getJoin attaches loaded relations as ordinary own properties, so without
   * this they were indistinguishable from real fields when serialising.
   *
   * @returns {Set<string>}
   */
  Model.relationNames = function () {
    const names = new Set();
    const groups = Model.relationships || {};
    ["hasMany", "belongsTo", "hasOne", "hasAndBelongsToMany"].forEach((kind) => {
      (groups[kind] || []).forEach((rel) => names.add(rel.relationName));
    });
    return names;
  };
   ...
   *
   * Relations are stripped. `save()` previously wrote `this` wholesale, so a
   * document loaded with getJoin had full copies of its children written back
   * into its own row on every edit - and because `linkedRequests` holds sibling
   * Requests that each already carry their own embedded copies, those grew
   * multiplicatively until they hit RethinkDB's document size limit.
   *
   * @param {object} instance
   * @returns {object}
   */
  function toDocument(instance) {
    const relations = Model.relationNames();
    const doc = {};
    Object.keys(instance).forEach((key) => {
      if (relations.has(key)) return;
      doc[key] = instance[key];
    });
    return doc;
  }

  /**
   * Throw if RethinkDB reported a write failure.
   *
   * The driver resolves successfully and reports problems in the result body,
   * so an update against a deleted document came back `{skipped: 1}` and a
   * rejected insert came back `{errors: 1}` - both of which used to look
   * exactly like success to the caller.
   *
   * @param {object} result
   * @param {string} operation
   * @param {boolean} [expectWrite] - whether the row must have been touched
   */
  function assertWritten(result, operation, expectWrite = true) {
    if (result && result.errors) {
      throw new Error(
        `${tableName} ${operation} failed: ${result.first_error || "unknown error"}`
      );
    }
    if (expectWrite && result && result.skipped) {
      throw new Error(
        `${tableName} ${operation} matched no document - it may have been deleted.`
      );
    }
  }
  ...
  Model.prototype.save = async function () {
    ...
    Object.keys(schema).forEach((key) => {
      if (
        ...
        if (typeof schema[key]._default === "function") {
          ...
          // Copied, never shared. Assigning the schema's default array by
          // reference meant a later `notes.push(...)` mutated the default
          // itself, so every subsequent document created without an explicit
          // value started life carrying that other request's note.
          this[key] = cloneDefault(schema[key]._default);
        }
      }
    });
    ...
    if (this.id) {
      const result = await r
        .table(tableName)
        .get(this.id)
        .update(toDocument(this))
        .run();
      assertWritten(result, "update");
    } else {
      const result = await r.table(tableName).insert(toDocument(this)).run();
      assertWritten(result, "insert", false);
      if (result.generated_keys && result.generated_keys.length > 0) {
